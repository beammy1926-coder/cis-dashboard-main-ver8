...
def render(ctx):
    ...
    </div><p style="color:#94A3B8; font-size:13.5px; line-height:1.4; margin:0;">อันดับที่ {sector_rank} จาก {n_sector} บริษัทในกลุ่ม {ctx.stock_info.get('sector','-')} จาก Overall Score = {safe(ctx.stock_info.get('overall_score')):.1f}/100
    {f'<br><span style="color:#F59E0B;">⚠️ กลุ่มนี้มีแค่ {ctx.selected_ticker} ตัวเดียวที่ติดตาม — เทียบกับทั้ง {n_all} หุ้นแทนด้วย: อันดับ {overall_rank}/{n_all}</span>' if single_member_sector else ''}
    </p>
    </div>""", unsafe_allow_html=True)
    ...
    {f'<div style="margin-top:8px; padding-top:8px; border-top:1px dashed #334155;"><div style="font-size:12px; color:#94A3B8;">เทียบทั้ง {n_all} หุ้น (กลุ่มมีตัวเดียว)</div><div style="font-size:18px; color:#38BDF8; font-weight:bold;">อันดับ {overall_rank}/{n_all}</div></div>' if single_member_sector else ''}
    </div>""", unsafe_allow_html=True)
